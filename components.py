...
def display_sidebar():
    ...
    with st.sidebar:
        # 利用目的見出し（h3風、余白ゼロ）
        st.markdown("<span style='font-size:1.3em; font-weight:bold; margin-bottom:0; display:block;'>利用目的</span>", unsafe_allow_html=True)
        # ラジオボタン（ラベル非表示、見出し直下）
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.warning(f"{ct.WARNING_ICON} 具体的に入力した方が期待通りの回答を得やすいです。")
